...
(define-struct bird-struct (y speed) #:transparent)
(define-struct pipe-struct (gap-y x) #:transparent)
(define-struct pipe-list (pipes) #:transparent)
(define-struct variables (gravity momentum scroll-speed) #:transparent)
(define-struct state-struct (bird pipes variables score) #:transparent)
;(define-struct pipe-list (pipe) #:transparent)

(define (get-initial-state)
  (let* ((variables (make-variables initial-gravity initial-momentum initial-scroll-speed))
         (bird (make-bird-struct bird-initial-y 0))
         (pipe (make-pipe-struct (+ added-number (random random-threshold)) scene-width))
         (pipe-list (make-pipe-list (list pipe))))
    (make-state-struct bird pipe-list variables 0)))

(define state-aux (get-initial-state))
(get-initial-state)
...
(define bird-aux (get-bird state-aux))
bird-aux
...
(define (next-state-bird bird gravity)
  (let* ((old-speed (bird-struct-speed bird))
         (new-speed (+ old-speed gravity))
         (old-y (bird-struct-y bird))
         (new-y (+ old-y old-speed)))
    (struct-copy bird-struct bird [y new-y] [speed new-speed])))

(next-state-bird bird-aux initial-gravity)
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-struct bird [speed (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (let* ((variables (state-struct-variables current-state))
         (momentum (variables-momentum variables))
         (bird (state-struct-bird current-state))
         (next-bird (next-state-bird-onspace bird momentum)))
  (if (key=? pressed-key " ")
      (struct-copy state-struct current-state [bird next-bird])
      current-state)))
...
(define (get-pipes state)
  (pipe-list-pipes (state-struct-pipes state)))

(define (get-pipes-struct state)
...
(define (copy-pipe scroll-speed)
  (λ (x) (struct-copy pipe-struct x [x (- (pipe-struct-x x) scroll-speed)])))

(define (move-pipes pipes scroll-speed)
  (map (copy-pipe scroll-speed) pipes))
...
(define (clean-pipes pipes)
  (if (< (pipe-struct-x (car pipes)) (- 0 pipe-width))
      (cdr pipes)
      pipes))
...
(define (pipe-builder pipes)
  (let* ((gap-y (+ added-number (random random-threshold)))
         (last-pipe (pipe-struct-x (car (reverse pipes))))
         (distance (+ pipe-width pipe-gap))
         (x (+ last-pipe distance)))  
  (make-pipe-struct gap-y x)))

  
(define (add-more-pipes pipes)
  (if (equal? (length pipes) no-pipes)
      pipes
      (append pipes (list (pipe-builder pipes)))))
...
(define (next-state-pipes pipes scroll-speed)
  (let* ((pipes-aux pipes)
         (moved (move-pipes pipes-aux scroll-speed))
         (cleaned (clean-pipes moved))
         (final (add-more-pipes cleaned)))    
    final))

;(next-state-pipes (state-struct-pipes state-aux) initial-scroll-speed)
...
(define (check-ground-collision bird)
  (let* ((bird-y (get-bird-y bird))
         (bird-down (+ bird-y bird-height)))
    (if (>= bird-down ground-y)
        #t
        #f)))
...
; sau cu pipes.
; Aceasta va primi ca paralmetru starea jocului.
...
(define (invalid-state? state)
  (let* ((bird (get-bird state))
         (pipes (pipe-list-pipes (get-pipes-struct state)))
         (ground-collision (check-ground-collision bird))
         (pipe-collision (check-pipe-collisions bird pipes)))
    (or pipe-collision ground-collision)))
...
(define (pipe-up-B1 pipe)
  (let* ((x (get-pipe-x pipe)))
    (make-posn x 0)))

(define (pipe-down-B1 pipe)
  (let* ((x (get-pipe-x pipe))
         (y (pipe-struct-gap-y pipe)))
    (make-posn (+ x pipe-width) y)))
         
(define (pipe-up-B2 pipe)
  (let* ((x (get-pipe-x pipe))
         (y (pipe-struct-gap-y pipe)))
    (make-posn x (+ y pipe-self-gap))))

(define (pipe-down-B2 pipe)
  (let* ((x (get-pipe-x pipe))
         (y (pipe-struct-gap-y pipe)))
    (make-posn (+ x pipe-width) scene-height))) 
     

(define (check-pipe-collisions bird pipes)
  (let collision ((A1 (make-posn bird-x (get-bird-y bird)))
         (A2 (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height)))
         (aux-pipes pipes))  
    (if (or (check-collision-rectangles A1 A2 (pipe-up-B1 (car aux-pipes)) (pipe-down-B1 (car aux-pipes)))
          (check-collision-rectangles A1 A2 (pipe-up-B2 (car aux-pipes)) (pipe-down-B2 (car aux-pipes))))
        #t
        (if (eq? (length aux-pipes) 1)
            #f
            (collision A1 A2 (cdr aux-pipes))))))
...
(define (score-inc state)
  (let* ((old-score (get-score state))
         (new-score (+ old-score 0.1)))
    new-score))

(define (next-state state)
  (let* ((bird (get-bird state))
         (pipes (get-pipes state))
         (variables (state-struct-variables state))
         (gravity (variables-gravity variables))
         (scroll-speed (variables-scroll-speed variables))
         (next-bird (next-state-bird bird gravity))
         (next-pipes (next-state-pipes pipes scroll-speed))
         (next-pipe-list (make-pipe-list next-pipes))
         (next-score (score-inc state)))
    
    (struct-copy state-struct state [bird next-bird] [pipes next-pipe-list] [score next-score])))

; draw-frame
; draw-frame va fi apelat de big-bang dupa fiecare apel la next-state, pentru a afisa cadrul curent.
;TODO 23
; Fiecare cadru va fi desenat in urmatorul mod:s
...
;(define pipe-draw (rectangle pipe-width pipe-height "solid" "green"))

(define aux-pipes (pipe-list-pipes (state-struct-pipes state-aux)))
aux-pipes
...
(define (draw-frame state)
  (let* ((pipe-scene-half (place-pipes-down state initial-scene))
         (pipe-scene (place-pipes-up state pipe-scene-half))
         (score-scene (place-image (score-to-image (state-struct-score state)) text-x text-y pipe-scene))
         (ground-scene (place-image ground (quotient scene-width 2) (+ ground-y (quotient ground-height 2)) score-scene))
         (final-scene (place-image bird (+ bird-x (quotient bird-width 2)) (+ (get-bird-y (get-bird state)) (quotient bird-height 2)) ground-scene)))
    final-scene))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.\
(define (draw-pipe-up pipe)
  (rectangle pipe-width (get-pipe-gap-y pipe) "solid" "green"))

(define (pipe-coord-up pipe)
  (make-posn (+ (get-pipe-x pipe) (quotient pipe-width 2)) (quotient (get-pipe-gap-y pipe) 2)))


(define (draw-pipe-down pipe)
  (rectangle pipe-width (- scene-height (+ pipe-self-gap (get-pipe-gap-y pipe))) "solid" "green"))

(define (pipe-coord-down pipe)
  (let* ((x (+ (get-pipe-x pipe) (quotient pipe-width 2)))
         (aux-y (- scene-height (+ pipe-self-gap (get-pipe-gap-y pipe))))
         (y (+ (+ (get-pipe-gap-y pipe) pipe-self-gap) (quotient aux-y 2))))
    (make-posn x y)))
  

(define (place-pipes-up pipes scene)
  (let* ((aux-pipes (pipe-list-pipes (state-struct-pipes pipes))))
    (place-images (map (λ (pp) (draw-pipe-up pp)) aux-pipes) (map (λ (pp) (pipe-coord-up pp)) aux-pipes) scene)))

(define (place-pipes-down pipes scene)
  (let* ((aux-pipes (pipe-list-pipes (state-struct-pipes pipes))))
    (place-images (map (λ (pp) (draw-pipe-down pp)) aux-pipes) (map (λ (pp) (pipe-coord-down pp)) aux-pipes) scene)))
